...
public class PaymentController : Controller
{
    ...
    {
        string webhookSecretKeyName = _configuration[key: "StripeSettings:WebhookSecret"] ?? throw new InvalidOperationException("Stripe webhook secret is not configured.");
        string? secret = Environment.GetEnvironmentVariable(webhookSecretKeyName);
        return secret;
    }
    ...
    private async Task HandleCancelledPaymentAsync(Event stripeEvent)
    {
        ...
        var paymentId = Int32.Parse(session.Metadata["PaymentId_DB"].ToString());
    ...
    private async Task HandlePaymentIntentFailureAsync(Event stripeEvent)
    {
        ...
        var paymentId = Int32.Parse(paymentIntent.Metadata["PaymentId_DB"].ToString());
    ...
    private async Task FulfillCheckout(string sessionId)
    {
        ...
        if (checkoutSession.PaymentStatus == "paid")
        {
            ConfirmPaymentDto confirm = new()
            {
                ...
                PaymentId = Int32.Parse(checkoutSession.Metadata["PaymentId_DB"].ToString()),
    ...
    {
        string stripeSecretKeyName = _configuration["StripeSettings:SecretKey"]
                       ?? throw new InvalidOperationException("Stripe secret key is not configured.");
        string key = Environment.GetEnvironmentVariable(stripeSecretKeyName)
