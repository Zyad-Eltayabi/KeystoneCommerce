...
namespace KeystoneCommerce.Application.Services
{
    public class CheckoutService : ICheckoutService
    {
        ...
        public async Task<Result<bool>> SubmitOrder(CreateOrderDto order)
        {
            try
            {
                ...
                {
                    return Result<bool>.Failure("The order must contain at least one product.");
                }
                ...
                if (!Enum.TryParse<PaymentType>(order.PaymentProvider, out var paymentType))
                {
                    ...
                    return Result<bool>.Failure("Invalid payment type.");
                }
                ...
                if (!orderCreationResult.IsSuccess)
                {
                    ...
                    return Result<bool>.Failure(orderCreationResult.Errors);
                }
                ...
                {
                    await _unitOfWork.RollbackAsync();
                    return Result<bool>.Failure(paymentResult.Errors);
                }
            ...
            catch (Exception ex)
            {
                ...
                return Result<bool>.Failure("An unexpected error occurred while processing your order. Please try again later.");
            }
        }
    }
}
